...
\fancyhead[C]{\small Potenzfunktionen - Wiederholungsarbeit \ (Kl. G10B)}
\fancyhead[R]{\small Name:\ \rule{2.8cm}{0.4pt}}
\fancyfoot[C]{\thepage}

\fancyfoot[C]{Seite \thepage \enspace\textbullet\enspace J.\,Mycan \textcopyright~19.Jan.2026 *Wiederholungsarbeit 45 min.*}
...
\newcommand{\noteVierMin}{\fpeval{round(\maxSumme * 0.50,0)}}
...
\begin{center}
	\begin{tabular}{cc}
		...
		% =========================================
% --- Graph (Potenzfunktion mit Exponent 1/2) ---
		\begin{minipage}[t]{0.18\textwidth}
			\begin{tikzpicture}[scale=0.8]
				% Koordinatensystem: x von -1 bis 4, y von -2 bis 4
				\draw[step=0.5,very thick,gray!60] (-2,-2) grid (4,4);
				...
				% Graph: f(x) = 1*sqrt(x+1) - 1
				\draw[domain=-1:4,smooth,thick,samples=200,variable=\x]
				plot ({\x},{1*sqrt(\x+1)-1});
			\end{tikzpicture}
		\end{minipage}%
		\hfill
		% --- Auswahl der Funktionsterme (angepasst) ---
		\begin{minipage}[t]{0.18\textwidth}
			\scriptsize
			\begin{tabular}{@{}l@{}}
				a) $f(x)=2\sqrt{x+1}-1$\\[2pt]
				b) $f(x)=2\sqrt{x}-1$\\[2pt]
				c) $f(x)=\sqrt{x+1}-1$\\[2pt]
				d) $f(x)=2\sqrt{x+1}+1$
			\end{tabular}
		\end{minipage}
		...
		% =========================================
% --- Potenz-/Bruchfunktion: Graph + Auswahl ---
\begin{minipage}[t]{0.48\textwidth}
	...
	\begin{minipage}[t]{0.58\textwidth}
		\begin{tikzpicture}[scale=0.8]
			...
			% (optional) Asymptoten dünn gestrichelt
			\draw[dashed,gray!70] (1,-2) -- (1,4);
			\draw[dashed,gray!70] (-4,1) -- (4,1);
			
			% Graph A: f(x) = -1/(x-1) + 1
			% Domain so gewählt, dass der Graph im Fenster y∈[-2,4] bleibt:
			% links: x <= 2/3  und rechts: x >= 4/3
			\draw[domain=-4:0.6667,smooth,thick,samples=200,variable=\x]
			plot ({\x},{-1/(\x-1) + 1});
			\draw[domain=1.3333:4,smooth,thick,samples=200,variable=\x]
			plot ({\x},{-1/(\x-1) + 1});
		\end{tikzpicture}
	\end{minipage}%
	\hfill
	...
	\begin{minipage}[t]{0.33\textwidth}
		\scriptsize
		\begin{tabular}{@{}l@{}}
			a) $f(x)= \phantom{-}\dfrac{1}{x-1}+1$\\[2pt]
			b) $f(x)= -\dfrac{1}{x-1}-1$\\[2pt]
			c) $f(x)= -\dfrac{1}{x-1}+1$\\[2pt]
			d) $f(x)= -\dfrac{1}{x+1}+1$
		\end{tabular}
	\end{minipage}
\end{minipage}
		...
		% =========================================
% --- Block: Kubische Potenzfunktion (Zuordnung) ---
\begin{minipage}[t]{0.48\textwidth}
	% linke Minipage: Graph
	\begin{minipage}[t]{0.58\textwidth}
		\begin{tikzpicture}[scale=0.8]
			% Grid und Achsen
			\draw[step=0.5,thick,gray!60] (-4,-2) grid (2,4);
			\draw[->] (-4,0) -- (2.2,0) node[right] {$x$};
			\draw[->] (0,-2) -- (0,4.2) node[above] {$y$};
			...
			% Graph A: f(x) = -(x+1)^3 + 1
			% Domain so gewählt, dass der Graph im Fenster y∈[-2,4] bleibt:
			\draw[domain=-2.44:0.44,smooth,thick,samples=200,variable=\x]
			plot ({\x},{-(\x+1)*(\x+1)*(\x+1) + 1});
		\end{tikzpicture}
	\end{minipage}%
	\hfill
	% rechte Minipage: Funktionsauswahl
...
\begin{minipage}[t]{0.48\textwidth}
	...
	\begin{minipage}[t]{0.58\textwidth}
		\begin{tikzpicture}[scale=0.8]
			% Grid und Achsen
			\draw[step=0.5,thick,gray!60] (-4,-2) grid (3,4);
			...
			% Asymptoten (optional, hilft beim Erkennen)
			\draw[dashed,gray!70] (-1,-2) -- (-1,4); % vertikale Asymptote x=-1
			\draw[dashed,gray!70] (-4,3) -- (4,3);   % horizontale Asymptote y=3
			
			% Graph A: f(x) = -1/(x+1)^2 + 3
			% Domain so gewählt, dass y >= -2 bleibt:
			% |x+1| >= 1/sqrt(5) ≈ 0.4472
			\draw[domain=-4:-1.4472,smooth,thick,samples=200,variable=\x]
			plot ({\x},{-1/((\x+1)*(\x+1)) + 3});
			\draw[domain=-0.5528:4,smooth,thick,samples=200,variable=\x]
			plot ({\x},{-1/((\x+1)*(\x+1)) + 3});
		\end{tikzpicture}
	\end{minipage}%
	\hfill
	...
	\begin{minipage}[t]{0.38\textwidth}
		...
		\begin{tabular}{@{}l@{}}
			...
			a) $f(x)= \phantom{-}\dfrac{1}{(x+1)^{2}}+3$\\[2pt]
			b) $f(x)= -\dfrac{1}{(x-1)^{2}}+3$\\[2pt]
			c) $f(x)= -\dfrac{2}{(x+1)^{2}}+3$\\[2pt]
			d) $f(x)= -\dfrac{1}{(x+1)^{2}}+3$
		\end{tabular}
	\end{minipage}
\end{minipage}
...
\end{center}
	...
	\begin{center}
		\begin{tabular}{|c|c|c|c|c|c|c|}
			...
			Note & 1 & 2 & 3 & 4 & 5 & 6 \\
			\hline
			Prozent \% & 100--95 & 94--80 & 79--65 & 64--50 & 49--16 & 15--0 \\
			\hline
			Punkte & \maxSumme{}--\noteEinsMin{} & \fpeval{\noteEinsMin-1}--\noteZweiMin{} & \fpeval{\noteZweiMin-1}--\noteDreiMin{} & \fpeval{\noteDreiMin-1}--\noteVierMin{} & \fpeval{\noteVierMin-1}--\noteFunfMin{} & \fpeval{\noteFunfMin-1}--\noteSechsMin{} \\
			\hline
		\end{tabular}
